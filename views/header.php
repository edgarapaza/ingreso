<!doctype html>
<html lang="es">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Archivo Central</title>
  <link rel="stylesheet" href="<?php echo constant('URL');?>public/css/main.css">
  <link rel="stylesheet" href="<?php echo constant('URL');?>public/css/footer.css">
  <link rel="stylesheet" href="<?php echo constant('URL');?>public/css/foundation.css">
  <link rel="stylesheet" href="<?php echo constant('URL');?>public/css/foundation-float.css">
  <link rel="stylesheet" href="<?php echo constant('URL');?>public/css/foundation-prototype.min.css">
  <script src="<?php echo constant('URL');?>public/js/jquery.js"></script>

</head>

<body>

  <div class="top-bar">
  <div class="top-bar-left">
    <ul class="dropdown menu" data-dropdown-menu>
      <li class="menu-text">Archivo Central</li>
      <li><a href="<?php echo constant('URL');?>main">Home</a></li>
      <li><a href="<?php echo constant('URL');?>lista">Lista</a></li>
      <li><a href="<?php echo constant('URL');?>rcco">RCCO</a></li>
      <li><a href="<?php echo constant('URL');?>rpresi">Res Presidencia</a></li>
    </ul>
  </div>
  <div class="top-bar-right">
    <ul class="menu">
      <li><a href="<?php echo constant('URL');?>salir/end">Salir</a></li>
    </ul>
  </div>
</div>
