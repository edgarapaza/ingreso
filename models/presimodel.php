<?php
class PresiModel extends Model
{
  ...
  {
    $sql = "SELECT idproyecto, idseccion, seccion, idserie, serie,metroslineales, totalunidades, estado, obs FROM proyecto GROUP BY idseccion;";
  ...
  function GuardarRcco($idcaja,$idproyecto,$numResolucion,$anio,$fecha,$resuelve,$totalfolios,$metroslineales,$idpersonal,$obs)
  {
    ...
    $dup = "SELECT COUNT(*) as numero FROM resolucionespresi WHERE  idcaja='$idcaja' AND idproyecto='$idproyecto' AND numResolucion='$numResolucion' AND anio='$anio' AND fecha='$fecha' AND resuelve='$resuelve';";
    $result = $this->conn->ConsultaArray($dup);

    if($result['numero'] > 0){
      $res = "Duplicado";
    }else{
      $sql = "INSERT INTO resolucionespresi VALUES (null,'$idcaja','$idproyecto','$numResolucion','$anio','$fecha','$resuelve','$totalfolios','$metroslineales','$fecCreate','$idpersonal','$obs',1);";
  ...
  function NoEncontrado($idcaja,$idproyecto,$numResolucion,$anio,$idpersonal)
  {
    ...
    $sql= "INSERT INTO resolucionespresi VALUES (null,'$idcaja','$idproyecto','$numResolucion','$anio','0000-00-00','No Encontrado',0,0,'$fecCreate','$idpersonal','',0);";
    $this->conn->ConsultaSin($sql);
